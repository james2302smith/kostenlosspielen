...
Class TP_Google_Analytics_Addon {
    ...
    {
	// Textdomain
	load_plugin_textdomain('tp-ga-addon', false, TP_ADDONS_PATH . DS . dirname(__FILE__) . '/languages/');
	// Menu
	...
	if ( isset($_REQUEST['tp_poll_id']) && $poll->id == $_REQUEST['tp_poll_id'] ):
		...
		if ( in_array($choice->id, (array) $_POST['tp_choices']) ):
		    // Send votes to GA
		    $ga_calls .= sprintf("ga('send', 'event', '[POLLS] %s', 'votes', '#%s', %d);", esc_attr($poll->question), $index + 1, $choice->votes);
		endif;
	    endforeach;
	    // Now concate
	    $ga = sprintf($script, $ga_calls, defined('WP_DEBUG') && WP_DEBUG === true ? '[TotalPoll][Google Analytics Addon]: Please setup Google Analytics.' : '');
	endif;
	return $content . $ga;
    }
