...
Class TP_Maximum_Selection_Addon {
    ...
    {
	// Textdomain
	load_plugin_textdomain('tp-ms-addon', false, TP_ADDONS_PATH . DS . dirname(__FILE__) . '/languages/');
	// Poll settings
	...
	if ( isset($poll->limitations->limit_maximum_answers) && intval($poll->limitations->maximum_answers) > 1 ):
	    ...
	    if ( count($_POST['tp_choices']) > $poll->limitations->maximum_answers ):
		// Render vote.php
    ...
    {
	wp_enqueue_script('tp-facebook-like', TP_ADDONS_URL . 'max-selection/helper.min.js', array( 'jquery' ));
    }
	...
	$message = sprintf('<p class="warning">%s</p>', sprintf(__("You can vote for up to %s choices.", 'tp-ms-addon'), $poll->limitations->maximum_answers));
