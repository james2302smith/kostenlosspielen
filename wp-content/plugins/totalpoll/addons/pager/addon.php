...
Class TP_Pager_Addon {
    ...
    {
	// Text domain
	load_plugin_textdomain('tp-pager-addon', false, TP_ADDONS_PATH . DS . dirname(__FILE__) . '/languages/');
	...
	<div class="tp-pagination">
	    <input type="hidden" name="tp_page" value="<?php echo $poll->current_page; ?>">
	    <input type="hidden" name="tp_showing" value="<?php echo isset($poll->showing_results) ? 'results' : 'vote'; ?>">
	    <button type="button" class="tp-btn tp-primary-btn tp-pagination-prev" name="tp_action" value="prev_page" <?php disabled(false, isset($poll->prev_page) && $poll->prev_page) ?>>
		...
		for ( $page = 1; $page <= $poll->total_pages; $page++ ):
		    ?>
		    <button type="button" class="tp-btn" name="tp_action" value="next_page" <?php disabled($poll->current_page, $page); ?> onclick="jQuery(this).parent().find('[name=\'tp_page\']').val(<?php echo $page - 1; ?>)">
			<?php echo $page; ?>
		    </button>
		    <?php
		endfor;
	    ...
	    ?>
	    <button type="button" class="tp-btn tp-primary-btn tp-pagination-next" name="tp_action" value="next_page" <?php disabled(false, isset($poll->next_page) && $poll->next_page) ?>>
