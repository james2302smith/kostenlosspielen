...
Class TP_Admin {
	...
	if ( $_POST ):
		...
		if ( isset($_POST['activate']) ):
		    update_site_option('tp_addons', array_merge($addons, TotalPoll()->addons->activated));
		endif;
	...
	if ( !empty($_POST['upgrade']) && is_array($_POST['upgrade']) && wp_verify_nonce($_POST['upgrade_polls_nonce'], plugin_basename(TP_ROOT_FILE)) ):
		...
		// IP Timeout
		$new_options['limitations']['ip']['timeout'] = isset($poll_security_extra['ip_timeout']) ? $poll_security_extra['ip_timeout'] : 15;

		// IP Range
		$new_options['limitations']['ip_range']['width'] = isset($poll_security_extras['ip_range_limit']) ? $poll_security_extras['ip_range_limit'] : 5;
		$new_options['limitations']['ip_range']['limits'] = isset($poll_security_extras['ip_range_votes_limit']) ? $poll_security_extras['ip_range_votes_limit'] : 5;

		// Quota
		$new_options['limitations']['revote']['quota'] = isset($poll_security_extras['votes_quota']) && $poll_security_extras['votes_quota'] != 0;
		$new_options['limitations']['quota'] = isset($poll_security_extras['votes_quota']) ? $poll_security_extras['votes_quota'] : 0;
