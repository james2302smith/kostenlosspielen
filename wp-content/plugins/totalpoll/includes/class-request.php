...
Class TP_Request {
	...
	if ( TotalPoll()->poll->load($_REQUEST['tp_poll_id']) ):
		...
		if ( TotalPoll()->security->has_ability_to_vote() && !empty($_POST['tp_choices']) ):
			...
			// Status for logs
			$status = __('Accepted', TP_TD);
		    else:
			$status = __('Denied', TP_TD);
		    endif;
		...
		if ( !empty($status) && !empty($voted_for) ):

		    $choices_indexes = array();
		    foreach ( $poll->choices as $index => $choice ):
			if ( in_array($choice->id, $voted_for) )
			    $choices_indexes[] = '#' . ($index + 1);
		    endforeach;
		    
		    $message = sprintf(
				__('[%s] vote for %s from %s using %s', TP_TD),
				$status,
				implode(', ', $choices_indexes),
				TotalPoll()->security->ip,
				$_SERVER['HTTP_USER_AGENT']
		    );
		    
		    TotalPoll()->logs->log($message);
