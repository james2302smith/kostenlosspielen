...
function video_sitemap_loop () {
    global $wpdb;
	$query='SELECT DISTINCT ID, post_title, post_name, post_date_gmt, f1.meta_value as image, f2.meta_value as intro, f3.meta_value as flash 
			...
			AND f3.meta_key =  \'flash\'
			ORDER BY post_date DESC'; 

    $posts = $wpdb->get_results ($query);
        $xml  = '<?xml version="1.0" encoding="UTF-8"?>' . "\n";       
        $xml .= '<!-- Created by '.SITE_ROOT_URL.'/ -->' . "\n";
        $xml .= '<!-- Generated-on="' . date("F j, Y, g:i a") .'" -->' . "\n";             
        $xml .= '<?xml-stylesheet type="text/xsl" href="'.SITE_ROOT_URL.'/wp-content/plugins/xml-sitemaps-for-videos/video-sitemap.xsl"?>' . "\n" ;
        $xml .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:video="http://www.google.com/schemas/sitemap-video/1.1">' . "\n";
        $videos = array();
        foreach ($posts as $post) {
				//if(strlen($post->flash)<30){ echo '--'.$post->flash.' with id='.$post->ID.'<br />';}
                if(strlen($post->flash)>30){ 
                $permalink = SITE_ROOT_URL."/".$post->post_name.".html";
                        $xml .= "\n <url>\n";
                        $xml .= " <loc>$permalink</loc>\n";
                        $xml .= " <video:video>\n";
                        $xml .= "  <video:content_loc>".$post->flash."</video:content_loc>\n";
                        $xml .= "  <video:thumbnail_loc>".$post->image."</video:thumbnail_loc>\n";
                        $xml .= "  <video:title>" . htmlspecialchars($post->post_title) . "</video:title>\n";
                        $xml .= "  <video:description>" . htmlspecialchars($post->intro) . "</video:description>\n";
		...
		foreach($categories as $category){
			...
			if($category->category_parent>0){
				...
				$permalink = "http://www.kostenlosspielen.biz/".$parent_category->slug."/".$category->slug."/";				
			}
			else{
				$permalink = "http://www.kostenlosspielen.biz/".$category->slug."/";				
			}
			$xml .= " <loc>$permalink</loc>\n";
                        $xml .= " <video:video>\n";
                        $xml .= "  <video:content_loc>".$permalink."</video:content_loc>\n";
                        $xml .= "  <video:thumbnail_loc>http://www.kostenlosspielen.biz/wp-content/themes/twentyten/styles/default/".$category->slug.".png</video:thumbnail_loc>\n";
                        $xml .= "  <video:title>" . htmlspecialchars($category->name) . "</video:title>\n";
                        $xml .= "  <video:description>" . htmlspecialchars($category->category_description) . "</video:description>\n";
			$xml .= " </video:video>\n </url>";
		}*/
